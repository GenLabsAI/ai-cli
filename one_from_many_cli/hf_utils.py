...
def list_experts(repo_id: str, token: Optional[str] = None) -> List[str]:
...
def download_expert_folder(repo_id: str, expert: str, dest_expert_dir: Path, token: Optional[str] = None) -> None:
    snapshot_path = Path(snapshot_download(repo_id=repo_id, allow_patterns=[f"experts/{expert}/*"], token=token))
...
def find_expert_index_file(expert_dir: Path) -> Optional[Path]:
    candidates = [
        ...
        expert_dir / "pytorch_model.bin.index.json",
    ]
    for c in candidates:
        if c.exists():
            return c
...
def rewrite_index_to_local(index_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    new_map = {k: posixpath.basename(v) for k, v in wm.items()}
    ...
    def _ensure_file_from_repo(repo_rel: str, dest: Path):
        if dry:
            if not dest.exists():
                dest.touch()
            return
        local_fp = hf_hub_download(repo_id=repo_id, filename=repo_rel, token=token)
        if not dest.exists():
